...
import type { AssistantResultResponse, RoutingRunEnvelope } from "../api/contracts"

type AppStateValue = {
  latestRun: RoutingRunEnvelope | null
  setLatestRun: (value: RoutingRunEnvelope | null) => void
  pinnedCharts: AssistantResultResponse[]
  pinChart: (value: AssistantResultResponse) => void
...
export function AppStateProvider({ children }: PropsWithChildren) {
  ...
  const [pinnedCharts, setPinnedCharts] = useState<AssistantResultResponse[]>([])
  ...
  const pinChart = useCallback((chart: AssistantResultResponse) => {
